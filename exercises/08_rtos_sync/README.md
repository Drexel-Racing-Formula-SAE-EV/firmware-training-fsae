...
Untouched starters compile, but checks intentionally report INCOMPLETE (and behavioral tests may fail). The student ELF is `build/student/08/project08.elf`. The reference remains `python tools/build.py 08` / `python tools/run.py 08`. Close an earlier Renode session before launching another.
...
Run `.\\tools\\debug.ps1 -Project 08 -Student`. At the initial main breakpoint, useful commands (enter separately) are:
