...
{
    /* TODO(08.1): For PA0 and an existing semaphore, increment events, give with FromISR, and request a yield if needed. */
    (void)pin;
}

static void sensor_task(void *argument)
{
    /* TODO(08.2): Every 100 ms enqueue sequence/time/raw by value with zero wait; count failed sends via record_queue_drop. */
    (void)argument;
...
{
    /* TODO(08.3): Honor flood_active using task notification, block on queue receive, and publish latest_sample inside a short critical section. */
    (void)argument;
    for (;;) { vTaskDelay(pdMS_TO_TICKS(100U)); }
}
