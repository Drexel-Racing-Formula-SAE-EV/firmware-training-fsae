...
{
    /* TODO(04.2): Publish rx_byte into the bounded SPSC ring before advancing rx_head; detect full, count overflow, mark corrupt, and keep RX rearming. */
    if (uart->Instance != UART4) { return; }
    /* Receive is rearmed so the unfinished exercise does not halt the IRQ. */
    if (HAL_UART_Receive_IT(&huart4, &rx_byte, 1U) != HAL_OK) {
        rx_recover = true; rx_corrupt = true; rx_errors++;
    }
}
