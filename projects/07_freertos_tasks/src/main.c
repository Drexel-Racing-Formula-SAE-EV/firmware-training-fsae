...
static void sensor_task(void *argument)
{
    ...
    for (;;) {
        uint32_t now = millis();
        uint16_t raw;
        if (adc_input_read(&raw) == ADC_INPUT_OK) {
            temperature_mc = (int32_t)(((uint32_t)raw * 330000U) / 4095U);
        }
        sensor_period_ms = now - previous;
        previous = now;
        sensor_count++;
...
static void control_task(void *argument)
{
    ...
    for (;;) {
        ...
        can_fresh = training_command_fresh(&command, now);
        while (consumed_buttons != button_events) {
            consumed_buttons++;
            board_led_toggle();
        }
        control_period_ms = now - previous;
        previous = now;
        control_count++;
